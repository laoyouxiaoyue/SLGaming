...
func RegisterSimpleGraph(ctx context.Context) {
	...
	r, err := g.Compile(ctx)
	if err != nil {
		logx.Errorf("compile graph failed, err=%v", err)
	...
	if err != nil {
		logx.Errorf("invoke graph failed, err=%v", err)
		return
	}

	logx.Infof("eino simple graph output is: %v", message)
}
...
func RegisterAnyInputGraph(ctx context.Context) {
	...
	logx.Infof("eino any input graph output is: %v", message)
}
