...
func (l *GetCompanionRatingRankingLogic) GetCompanionRatingRanking(in *user.GetCompanionRatingRankingRequest) (*user.GetCompanionRatingRankingResponse, error) {
	...
	// ZrevrangeWithScores 返回带分数的成员列表
	key := "ranking:rating"
	members, err := l.svcCtx.Redis.ZrevrangeWithScores(key, int64(start), int64(end))
	if err != nil {
		l.Errorf("query rating ranking from redis failed: %v", err)
	...
	if err != nil {
		l.Errorf("get rating ranking total from redis failed: %v", err)
	...
	for i, member := range members {
		...
		rating := float64(member.Score) / 10000.0
