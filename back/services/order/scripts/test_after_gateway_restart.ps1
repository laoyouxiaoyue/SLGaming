# Test script - Run after Gateway service restart
...
function Decode-JWT {
    ...
    while ($p.Length % 4) { $p += "=" }
    return ([System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($p)) | ConvertFrom-Json).user_id
}

Write-Host "=== Complete Order Cancel Test (After Gateway Restart) ===" -ForegroundColor Cyan
Write-Host ""
...
try {
    $profile = Invoke-Api -Method Put -Path "/api/user/companion/profile" `
        -Headers @{Authorization="Bearer $COMP_TOKEN"} `
        -Body @{gameSkills='["Game1"]'; pricePerHour=100; status=1}
    if ($profile.code -eq 0) {
        Write-Host "OK Profile set (Price: $($profile.data.pricePerHour))" -ForegroundColor Green
    } else {
        Write-Host "FAIL: $($profile.msg)" -ForegroundColor Red
        Write-Host "If you see '未登录或认证失败', check Gateway logs for middleware extraction" -ForegroundColor Yellow
        exit
    }
} catch {
...
$order1 = Invoke-Api -Method Post -Path "/api/order" `
    -Headers @{Authorization="Bearer $BOSS_TOKEN"} `
    -Body @{companionId=[int64]$COMP_ID; gameName="Game1"; durationMinutes=60}
if ($order1.code -ne 0) {
    Write-Host "FAIL Create order: $($order1.msg)" -ForegroundColor Red
    exit
}
...
$cancel1 = Invoke-Api -Method Post -Path "/api/order/cancel" `
    -Headers @{Authorization="Bearer $BOSS_TOKEN"} `
    -Body @{orderId=[int64]$ORDER_ID1; reason="Test cancel"}
...
$order2 = Invoke-Api -Method Post -Path "/api/order" `
    -Headers @{Authorization="Bearer $BOSS_TOKEN"} `
    -Body @{companionId=[int64]$COMP_ID; gameName="Game2"; durationMinutes=60}
if ($order2.code -ne 0) {
    Write-Host "FAIL Create order: $($order2.msg)" -ForegroundColor Red
    exit
}
...
try {
    $cancel2 = Invoke-Api -Method Post -Path "/api/order/cancel" `
        -Headers @{Authorization="Bearer $BOSS_TOKEN"} `
...
$cancel3 = Invoke-Api -Method Post -Path "/api/order/cancel" `
    -Headers @{Authorization="Bearer $COMP_TOKEN"} `
    -Body @{orderId=[int64]$ORDER_ID2; reason="Test"}
