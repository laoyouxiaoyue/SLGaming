...
# Register/Login
Write-Host "[1] Register/Login Boss..." -ForegroundColor Yellow
try {
    $regBoss = Invoke-Api -Method Post -Path "/api/user/register" -Body @{phone="13800001001"; code="123456"; password="Test123456"; nickname="Boss"; role=1}
    if ($regBoss.code -eq 0) {
        $BOSS_TOKEN = $regBoss.data.accessToken
        $BOSS_ID = $regBoss.data.id
        Write-Host "OK Boss registered (ID: $BOSS_ID)" -ForegroundColor Green
    } else {
        $loginBoss = Invoke-Api -Method Post -Path "/api/user/login" -Body @{phone="13800001001"; password="Test123456"}
...
Write-Host "[2] Register/Login Companion..." -ForegroundColor Yellow
try {
    $regComp = Invoke-Api -Method Post -Path "/api/user/register" -Body @{phone="13800002001"; code="123456"; password="Test123456"; nickname="Companion"; role=2}
    if ($regComp.code -eq 0) {
        $COMP_TOKEN = $regComp.data.accessToken
        $COMP_ID = $regComp.data.id
        Write-Host "OK Companion registered (ID: $COMP_ID)" -ForegroundColor Green
    } else {
        $loginComp = Invoke-Api -Method Post -Path "/api/user/login" -Body @{phone="13800002001"; password="Test123456"}
