...
type TokenStore interface {
	...
	// RevokeAllUserTokens 撤销用户的所有 Refresh Token（设置用户级别黑名单）
...
func (r *RedisTokenStore) VerifyRefreshToken(ctx context.Context, userID uint64, token string, tokenExpiration time.Time) (bool, error) {
	...
	// 2. 检查具体 token 的黑名单
	tokenBlacklistKey := r.getTokenBlacklistKey(userID, token)
...
func (r *RedisTokenStore) RevokeAllUserTokens(ctx context.Context, userID uint64, refreshTokenDuration time.Duration) error {
	...
	if err == nil {
		logx.Infof("Revoked all refresh tokens for user %d, TTL: %d seconds", userID, ttl)
	}
	return err
}

// getTokenBlacklistKey 获取 token 黑名单的 Redis key
func (r *RedisTokenStore) getTokenBlacklistKey(userID uint64, token string) string {
